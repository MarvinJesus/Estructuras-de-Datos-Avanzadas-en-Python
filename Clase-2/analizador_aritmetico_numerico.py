...
class AnalizadorAritmeticoNumerico:
    ...
    def _estructura_ok(self, tokens: list) -> bool:
        """Comprueba: operando (operador operando)*; operando = N o ( subexpresión )."""
        if not tokens:
            return False
        pila = []
        need_op = True
    ...
    def validar(self, expresion: str) -> bool:
        ...
        if not expresion or not expresion.strip():
            return False
        tokens = self._tokenizar(expresion.strip())
        return tokens is not None and self._estructura_ok(tokens)
